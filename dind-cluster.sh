...
if [[ ${DIND_COMMIT} ]]; then
  if [[ ${DIND_COMMIT} = current ]]; then
    DIND_COMMIT="$(git rev-parse HEAD)"
  fi
  DIND_K8S_VERSION="${DIND_K8S_VERSION:-v1.13}"
  DIND_IMAGE="${DIND_IMAGE:-${DIND_IMAGE_BASE}:${DIND_COMMIT}-${DIND_K8S_VERSION}}"
else
...
function dind::ensure-downloaded-kubectl {
  ...
  source <(docker run --entrypoint /bin/bash --rm "${DIND_IMAGE}" -c "cat /dind-env")
